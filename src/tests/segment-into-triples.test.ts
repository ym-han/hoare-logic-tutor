...
import { AlrContext, Alr, type AlrNodeInfo } from '$lib/alr-core/index.ts';
import { HLProofExercise, HLProofExerciseDataSource, makeAssertionHole } from '$lib/hoare-logic-specific/data/index.ts';
...
describe('parse into triples -- length', () => {
	it('most basic x skip', () => {
		...
		const exerciseAlr = HLProofExerciseDataSource.toAlr(
			nodeInfo,
			new HLProofExercise(proofSteps, new DummyExerciseFeedbackGiver())
		);

		// console.log(`getProofSteps: ${exerciseAlr.getProofSteps(nodeInfo.context).length}`);
		const htNodes = parseIntoHoareTriples(
			nodeInfo,
			exerciseAlr.getProofSteps(nodeInfo.context).filter(isCoreProofStepAlrNode)
		);

		expect(htNodes.length).toBe(1);
		expect(htNodes[0]?.getHoareTriple(nodeInfo.context)?.getCommand().toString()).toBe(
			parseToCommand('skip;').toString()
		);
	});
