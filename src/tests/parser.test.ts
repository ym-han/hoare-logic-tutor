...
describe('Basic commands', () => {
	it('Assignment: var, rhs, substitution', () => {
		...
		expect(
			assignment
				.getVar()
				.subst(new Var('x' as Name), new NumLit(1337))
				.toString()
		).toBe('1337');
...
describe('Unary operators', () => {
	it('operator precedence', () => {
		const formula = parseToAssertion('{ -2 + 1 = 0 }').getFormula()
		expect(formula.toString()).toBe('(((-1 * 2) + 1) = 0)')
	})
	it('Negation of number', () => {
			const formula = parseToAssertion('{ -(+5) = +(-5) }').getFormula();
			expect(formula.toString()).toBe('((-1 * 5) = (-1 * 5))');
	});

	it('Double negation of number', () => {
			const formula = parseToAssertion('{ --5 = 5 }').getFormula();
			expect(formula.toString()).toBe('((-1 * (-1 * 5)) = 5)');
	});
});
...
describe('Binary operators', () => {
	...
	it('Left-associative conjunction', () => {
		expect(parseToAssertion('{ true && !false && !!true }').getFormula().toString()).toEqual(
			'((true && !false) && !!true)'
		);
	});
