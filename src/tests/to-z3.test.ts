...
import { type Z3Env, killThreads, z3ProveConjecture } from '../lib/lang-support/index.ts';
...
describe('Imp Assertion to Z3 translation', () => {
    ...
    it('Logical operators', async () => {
        // { true && false } is unsat
        expect(await z3env.z3context.solve(parseToAssertion('{ true && false }').toZ3(z3env) as Z3.Bool)).toBe('unsat');
    ...
    // it('Unary negation tautology', async () => {
    //     const tautology = parseToAssertion('{ --5 = 5 }').toZ3(z3env);
    //     expect(await z3env.z3context.solve(z3env.z3context.Not(tautology))).toBe('unsat');
    // });
});
...
describe('Adapted from HLT exercises', () => {
    ...
    it('Adapted from HLT unit. Not equiv.', async () => {
        ...
        // Not equiv
        const formula1 = parseToAssertion('{ b > 0 && ( (a <= 0) => x = 8*b + 1) && ( (a > 0) => x = 12*b + 1) }').toZ3(
            z3env
        );
    ...
    it('Adapted from HLT EG 4', async () => {
        ...
        const consequent = parseToAssertion(
            '{ b > 0 && ( a <= 0 => (((2 - (a + 1) / a) / 2) * 2 + (1 - ((2 - (a + 1) / a) / 2)) * 3) = 2 ) && ( a > 0 => (((2 - (a + 1) / a) / 2) * 2 + (1 - ((2 - (a + 1) / a) / 2)) * 3) = 3 ) }'
        );
...
describe('Division semantics', () => {
  ...
  *******************************/
    ...
    it('negative/negative integer division rounds to 0: (-2 + 1) / -2 = 0', async () => {
		expect(parseToAssertion('{ (-2 + 1) / -2 = 0 }').getFormula().toString()).toBe('((((-1 * 2) + 1) / (-1 * 2)) = 0)')
...
describe('checkHoareTriple with edge case triples', () => {
    it('Vacuous precond 1: {false} S {true}', async () => {
        const { result } = await checkHoareTriple(
            z3env,
            new HoareTriple(parseToAssertion('{ false} '), parseToCommand('x := 99999999999 '), parseToAssertion('{ true }'))
        );
        expect(result).toStrictEqual('unsat');
    });
    ...
    it('True implies true: {true} skip {true}', async () => {
        const triple = new HoareTriple(parseToAssertion('{ true }'), parseToCommand('skip;'), parseToAssertion('{ true }'));
        const { result } = await checkHoareTriple(z3env, triple);
        expect(result).toBe('unsat');
    });
