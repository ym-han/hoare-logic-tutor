...
export class Assign extends BaseToplevelDecl {
  constructor(
    ...
    private readonly rhs: ArithExpr
  ) {
    super(parserInfo);
  }
...
abstract class BaseExpr {
  ...
  getFreeProgramVars(): Var[] {
    return [...new Set(this.getChildren().flatMap((child) => child.getFreeProgramVars()))];
  }

  abstract getChildren(): Expr[];

  abstract map(transformFormula: (f: Formula) => Formula, transformArith: (a: ArithExpr) => ArithExpr): this;
}
...
export class Assertion extends BaseToplevelDecl {
  ...
  map(transformFormula: (f: Formula) => Formula, transformArith: (a: ArithExpr) => ArithExpr): this {
...
export type Formula = BoolLit | PredicateApp | Not | And | Or | Implies | Eq | Neq | Lt | Lte | Gt | Gte;
...
export class PredicateApp extends BaseFormula {
  ...
  toZ3(z3env: Z3Env) {
    ...
    const predicate = z3env.z3context.Function.declare(this.getPredicateName() as string, ...argSorts, returnSort);
  ...
  map(_transformFormula: (f: Formula) => Formula, transformArith: (a: ArithExpr) => ArithExpr): this {
    return new PredicateApp(this.getPredicateName(), this.getArgs().map(transformArith) as ArithExpr[]) as this;
  }
}
...
export class Not extends BaseFormula {
  ...
  subst(v: Var, expr: ArithExpr): Formula {
    return new (this.constructor as new (formula: Formula) => this)(this.getNegand().subst(v, expr));
  }
  ...
  map(transformFormula: (f: Formula) => Formula, _transformArith: (a: ArithExpr) => ArithExpr): this {
...
export class BoolLit extends BaseFormula {
  ...
  map(_transformFormula: (f: Formula) => Formula, _transformArith: (a: ArithExpr) => ArithExpr): this {
    return this;
  }
...
export type ArithExpr = NumLit | Var | Plus | Minus | Mult | Divide;
...
function makeZ3Div(z3env: Z3Env, dividend: Z3.Arith<'main'>, divisor: Z3.Arith<'main'>) {
  ...
  return If(
    ...
    If(
      ...
      If(divisor.ge(zero), dividend.div(divisor).add(one), dividend.div(divisor).sub(one))
    )
  );
}
