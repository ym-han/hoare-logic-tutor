 ...
 */
export function concreteSyntaxToAbstractSyntax(cursor: TreeCursor, parserInfo: ParserInfo, src: string): Decl {
  return match(cursor.node.type.name)
    .with('TopDecl', () => {
      ...
      const decl = match(cursor.node.type.name)
        .with('Assertion', () => {
          ...
          const conjoinedFormula = commaSepFormulas.reduceRight((acc, newFormula) => new And(newFormula, acc));
    ...
    .otherwise(() => {
      throw new RefineLezerParseError(
        'Expected a top-level declaration (i.e., a Assertion or Command) but got',
        cursor,
        src
      );
    });
...
export function translateFormula(cursor: TreeCursor, src: string): Formula {
  ...
  return match(nodeType)
    ...
    .with('ParenthesizedFormula', () => walkParenthesizedExpression(cursor, src, translateFormula))
    .otherwise(() => {
...
function translateArithmeticExpression(cursor: TreeCursor, src: string): ArithExpr {
  ...
  return match(nodeType)
    ...
    .with('ParenthesizedArithExp', () => walkParenthesizedExpression(cursor, src, translateArithmeticExpression))
    .otherwise(() => {
...
function walkBinTreeCursor<Branch>(cursor: TreeCursor, src: string, dispatchBranch: BranchDispatcher<Branch>) {
  cursor.firstChild();
...
function parseArgList(cursor: TreeCursor, src: string): ArithExpr[] {
  ...
  while (cursor.nextSibling()) {
    ...
    try {
      ...
      throw new RefineLezerParseError(
        'Could not parse argument to predicate. Predicate arguments must be integer expressions.',
        cursor,
        src
      );
    }
  ...
  const desugarFormula = (formula: Formula): Formula => {
    return match(formula)
      .with(P.instanceOf(PredicateApp), (predApp) => {
        ...
        if (isInterpretedFunction(predName) && predApp.getArgs().length === INTERPRETED_FUNCTIONS[predName].arity) {
...
export class RefineLezerParseError extends Error {
  ...
  constructor(message: string, cursor: TreeCursor, src: string) {
    const augmentedMessage = `${cursor.node.from}-${cursor.node.to}: ${message} \`${src.substring(cursor.from, cursor.to)}\`.`;
