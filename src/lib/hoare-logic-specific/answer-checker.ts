import {
  ...
  IncorrectAttemptFeedbackAlrNode,
  CorrectAttemptFeedbackAlrNode,
  isIncorrectAttemptFeedbackAlrNode,
  NodeInfoManager
} from '$lib/alr-core/index.ts';
import type { AlrContext, ExerciseAlrNode, AlrNodeInfo, HasFeedback } from '$lib/alr-core/index.ts';
...
import {
  ...
  CommandAlrNode,
  isCommandAlrNode,
  isCoreProofStepAlrNode,
  type CoreProofStepAlrNode,
  type HasAssertion,
  hasAssertion
...
export class AssertionPrevalidator extends NodeInfoManager implements AnswerPrevalidator {
  constructor(readonly nodeInfo: AlrNodeInfo) {
    super(nodeInfo);
  }
...
export class HLProofExerciseZ3Checker extends NodeInfoManager implements ExerciseFeedbackGiver {
  ...
  async giveAndShowFeedback(
    context: AlrContext,
    exercise: HLProofExerciseAlrNode | HLScaffoldedProofExerciseAlrNode
  ): Promise<void> {
    const nodeInfo = this.makeNodeInfo(context);
    ...
    reversedTriples.forEach((ht) => ht.setEmptyFeedback(context));
...
export class IsLegitHoareTripleZ3Checker extends NodeInfoManager implements AnswerChecker<HoareTriple> {
  ...
  async check(context: AlrContext, triple: HoareTriple): Promise<FeedbackAlrNode> {
    ...
    return match(solverResult)
      .with('unsat', () => CorrectAttemptFeedbackAlrNode.make(nodeInfo, new Feedback(SAY_CORRECT_MESSAGE)))
      .otherwise(() => {
...
export class IsLegitHoareTripleAndNoFalsePrecondZ3Checker
  extends IsLegitHoareTripleZ3Checker
  implements AnswerChecker<HoareTriple>
{
  constructor(
    nodeInfo: AlrNodeInfo,
    protected readonly env: Z3Env
...
function makeWrongAttemptFeedbackMessage(triple: HoareTriple, preState: ProgramState, postState: ProgramState) {
  ...
  const postMessage =
    triple.getCommand() instanceof Skip
      ? codeBlock`
        But the postcondition
...
function computePreAndPostStatesAccordingToCountermodel(triple: HoareTriple, env: Z3Env, model: Z3.Model<'main'>) {
  ...
  const preAndPostCondVars = _.uniqBy(
    [...triple.getPre().getFormula().getFreeProgramVars(), ...triple.getPost().getFormula().getFreeProgramVars()],
    (v) => v.toString()
  );

  const preStateVars =
    triple.getCommand() instanceof Assign
      ? preAndPostCondVars.filter((v) => !v.isEqualTo((triple.getCommand() as Assign).getVar()))
      : preAndPostCondVars;
  const preState = Object.fromEntries(Array.from(preStateVars).map((v) => [v.toString(), model.eval(v.toZ3(env))]));

  const postState = match(triple.getCommand())
    .with(P.instanceOf(Assign), () => {
      const [assignVarName, assignRhs] = [
        (triple.getCommand() as Assign).getVar().toString(),
        (triple.getCommand() as Assign).getRhs()
      ];

      return { ...preState, [assignVarName]: model.eval(assignRhs.toZ3(env)) };
    })
 ...
 */
export async function checkHoareTriple(
  env: Z3Env,
  triple: HoareTriple
): Promise<{ solver: Z3.Solver<'main'>; result: Z3.CheckSatResult }> {
  ...
  return (
    match(command)
      ...
      .with(P.instanceOf(Assign), async (_assign) => {
        ...
        const conjecture = env.z3context.Implies(pre.toZ3(env), reducedPost.toZ3(env));
...
function step(post: Formula, cmd: Command): Formula {
  return (
    match(cmd)
      .with(P.instanceOf(Assign), (assign) => post.subst(assign.getVar(), assign.getRhs()))
 ...
 */
function naivelySegmentIntoHoareTriples(
  nodeInfo: AlrNodeInfo,
  proofSteps: CoreProofStepAlrNode[]
): HoareTripleAlrNode[] {
  ...
  return windows
    ...
    .map(
      ([pre, cmd, post]) =>
        new HoareTripleAlrNode(
          nodeInfo,
          pre as HasAssertion & HasFeedback,
          cmd as CommandAlrNode,
          post as HasAssertion & HasFeedback
        )
    );
}
