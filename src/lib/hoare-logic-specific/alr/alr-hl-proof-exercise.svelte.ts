...
import type { AlrId, AlrNode, ExerciseAlrNode, DefaultAlrNodeInfo, PlainStrContentAlrNode, HasFeedback, Unsubscriber } from '$lib/alr-core/index.ts';
...
export class HLProofExerciseAlrNode extends DefaultAlrNode implements ExerciseAlrNode {
  ...
  constructor(
    defaultNodeInfo: DefaultAlrNodeInfo,
...
export class HoleAlrNode<T extends HoleType> extends DefaultAlrNode implements HasStudentAttempt {
  ...
  constructor(defaultNodeInfo: DefaultAlrNodeInfo, hole: Hole<T>, readonly holeType: T) {
    super(defaultNodeInfo);
    this.#hole = hole;
  }
...
export class GivenAssertionAlrNode extends DefaultAlrNode implements HasAssertion, HasFeedback {
  ...
  static make(defaultNodeInfo: DefaultAlrNodeInfo, givenAssertion: Assertion): GivenAssertionAlrNode {
    return new GivenAssertionAlrNode(defaultNodeInfo, givenAssertion, PlainStrContentSource.toAlr(defaultNodeInfo,
...
export class CommandAlrNode extends DefaultAlrNode implements HasFeedback {
  ...
  static make(nodeInfo: DefaultAlrNodeInfo, command: Command): CommandAlrNode {
    return new CommandAlrNode(nodeInfo, command, PlainStrContentSource.toAlr(nodeInfo, command.getOriginalInput()));
  }

  private constructor(nodeInfo: DefaultAlrNodeInfo, command: Command, body: PlainStrContentAlrNode, feedback: FeedbackAlrNode = new EmptyFeedbackAlrNode(nodeInfo)) {
    super(nodeInfo);
    this.#body = body.getId();
...
export class RemarkAlrNode extends DefaultAlrNode {
  ...
  static make(nodeInfo: DefaultAlrNodeInfo, remark: Remark): RemarkAlrNode {
    return new RemarkAlrNode(nodeInfo, remark, PlainStrContentSource.toAlr(nodeInfo, remark.getBody()));
  }

  private constructor(nodeInfo: DefaultAlrNodeInfo, remark: Remark, body: PlainStrContentAlrNode) {
    super(nodeInfo);
    this.#body = body.getId();
    this.#remark = remark;
  }
