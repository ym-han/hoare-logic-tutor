...
export interface AlrSource<A, B extends AlrNode> {
  toAlr(nodeInfo: AlrNodeInfo, data: A): B;
}
...
export interface ExerciseAlrNode extends AlrNode {}

// -----------------------------------------------
//            Node Info related
// -----------------------------------------------

export interface AlrNodeInfo {
  alr: Alr;
  context: AlrContext;
}

export type AlrNodeInfoWithoutContext = Omit<AlrNodeInfo, 'context'>;
...
export function setAlrInSvelteContext(alr: ReturnType<typeof getAlrFromSvelteContext>) {
  const env: DisplayerInfo = {
    getAlr() {
      return alr;
    }
  };
