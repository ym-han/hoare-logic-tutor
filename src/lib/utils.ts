...
/** aka Maybe */
export type Optional<T> = { type: 'success'; value: T } | { type: 'failure' };
 ...
 * ```
 */
export function liftToOptional<T>(
	op: (result1: T, result2: T) => T
): (mr1: Optional<T>, mr2: Optional<T>) => Optional<T> {
	return (mr1: Optional<T>, mr2: Optional<T>) => {
		return match([mr1, mr2])
			.with([{ type: 'success' }, { type: 'success' }], ([{ value: r1 }, { value: r2 }]) => makeJust(op(r1, r2)))
...
export function cleanupBaseCodeMirrorWrapper(baseCMWrapper: CodeMirror) {
	if (baseCMWrapper && baseCMWrapper.getRef() && baseCMWrapper.getRef().getEditorView()) unmount(baseCMWrapper);
}
